...
contract AmmagBank is IAccounts{
   ...
   struct Account {
    ...
    function DepositToken(uint amount) public override IsdeListed blackListedAddress  {      
        uint allowance = token.allowance(msg.sender, address(this));
        require(allowance >= amount, "amount must be equal or less than allowance");
        token.transferFrom(msg.sender, address(this), amount);
        accounts[msg.sender].token += amount; 
    }

    function TokensWithdraw(uint256 _value, address tokenAddress) public override blackListedAddress lockedDeposit IsdeListed payable{
        require(accounts[msg.sender].token >= _value && _value >0 , "no sufficent amount.");
        accounts[msg.sender].token -= _value;
        //payable(msg.sender).transfer(_value); //ya tu token walay contract mai janay chahaya.
        token.transfer(tokenAddress, _value);
    }
    function showTokens(address accounterHolder) public view returns (uint Tokens){
        return accounts[accounterHolder].token; 
    }
